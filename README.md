...
SmartPromo pode ser adicionado no seu projeto `Android` utilizando o `Gradle`, para isto adicione a dependência abaixo ao arquivo `build.gradle` a nível de `módulo`:
    ...
    implementation 'br.com.getmo:smartpromo:3.0.1'
...
## Utilização
### Basico
Para começar a utilizar o SmartPromo, instancie a SDK passando suas `chaves de acesso`:

    val smartPromo = SmartPromo(accessKey = "{accessKey}", secretKey = "{secretKey}")

Para utilizar o ambiente de homologação, passe o parâmetro opcional `isHomolog`:

    val smartPromo = SmartPromo(accessKey = "{accessKey}", secretKey = "{secretKey}", isHomolog = true)
...
#### Iniciando a SDK no modo Scanner de notas:
    smartPromo.goScan(campaignId = "{campaignID}", consumerId = "{consumerID}", activity = {activity})
